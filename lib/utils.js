...
export const install = async ({ pnp }, { verboseSetup, date }) => {
	...
	try {
		if (pnp) {
			...
			await runCommand(
				...
				[
					...
					"set",
					"npmRegistryServer",
					date ? "http://localhost:3333" : "https://registry.yarnpkg.com",
				],
				verboseSetup
			);
			if (date) {
			...
			await runCommand(
				"yarn",
				date
					? ["config", "set", "registry", "http://localhost:3333"]
		...
		await runCommand("yarn", ["install"], verboseSetup);
	} finally {
		if (pnp) {
			await runCommand(
				"yarn",
			...
			await runCommand("yarn", ["config", "delete", "registry"], verboseSetup);
		}
		if (close) {
			await close();
		}
